...
func newDNSCache(max int) *dnsCache {
	return &dnsCache{data: make(map[string]*cacheEntry, max), order: list.New(), max: max}
}
...
// get returns cached data if present and promotes the entry
...
func (c *dnsCache) set(k string, data []byte, ttl int64) {
	...
	if len(c.data) >= c.max && c.max > 0 {
		...
		if front := c.order.Front(); front != nil {
			oldk := front.Value.(string)
			c.order.Remove(front)
			delete(c.data, oldk)
		}
	}
	elem := c.order.PushBack(k)
	c.data[k] = &cacheEntry{data: data, exp: nowUnix() + ttl, elem: elem}
}
